...
impl VaultsFilter {
    pub const ALL: [VaultsFilter; 5] = [
        VaultsFilter::Current,
        VaultsFilter::Active,
        VaultsFilter::Inactive,
        VaultsFilter::Moving,
        VaultsFilter::Moved,
    ];

    pub fn new(statuses: &[VaultStatus]) -> VaultsFilter {
        if statuses == VaultStatus::ACTIVE {
            return VaultsFilter::Active;
        } else if statuses == VaultStatus::INACTIVE {
            return VaultsFilter::Inactive;
        } else if statuses == VaultStatus::MOVING {
    ...
    pub fn statuses(&self) -> &'static [VaultStatus] {
        match self {
            Self::Active => &VaultStatus::ACTIVE,
            Self::Current => &VaultStatus::CURRENT,
            Self::Inactive => &VaultStatus::INACTIVE,
...
impl std::fmt::Display for VaultsFilter {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::Active => write!(f, "Delegated"),
            Self::Current => write!(f, "Current"),
            Self::Inactive => write!(f, "Not delegated"),
